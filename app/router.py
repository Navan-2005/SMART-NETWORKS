...
def send_packet(target_ip, pkt):
    try:
        ...
        # ✅ BIND TO LOOPBACK IP 
        # This ensures the packet source is 10.0.x.x, not 0.0.0.0
        sock.bind((MY_IP, 0))
...
def handle_packet(data, addr):
    ...
    # 1. HANDLE FEEDBACK (Learning Update)
    ...
    if next_hop:
        ...
        # Send Feedback to previous node
...
def start_router():
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    # Bind to 0.0.0.0 so we can receive from Localhost AND Neighbors
